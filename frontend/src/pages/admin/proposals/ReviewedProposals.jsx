    ...
    { key: 'pi', label: 'Principal Investigator', render: (row) => `${row.pi_first_name} ${row.pi_last_name}` },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="primary" onClick={() => openDecisionModal(row)}>
            Make Decision
          </Button>
        </div>
      ),
      ...
      >
          ...
          {DECISIONS.map((d) => (
            <button
              ...
              className={`flex-1 py-3 rounded-lg border-2 text-sm font-semibold transition ${
                decision === d
                  ? d === 'Approved' ? 'border-success bg-success/10 text-success'
                  : d === 'Rejected' ? 'border-danger bg-danger/10 text-danger'
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-textMain mb-1">
            Note to Applicant <span className="text-muted font-normal">(optional)</span>
          </label>
          <textarea
            value={note}
