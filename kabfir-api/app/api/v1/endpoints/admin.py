...
from app.schemas.schemas import (
    ...
    MessageResponse, SetReviewDeadlineRequest
)
...
@router.get("/proposals/awarded", response_model=List[ProposalResponse])
async def get_awarded_proposals(
    current_user: User = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(Proposal)
        .where(Proposal.status == ProposalStatus.awarded)
        .order_by(Proposal.admin_decision_at.desc())
    )
    return result.scalars().all()


@router.patch("/proposals/{proposal_id}/reviewers/{reviewer_id}/deadline", response_model=MessageResponse)
async def set_review_deadline(
    proposal_id: int,
    reviewer_id: int,
    payload: SetReviewDeadlineRequest,
    current_user: User = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
    """Set or update the review deadline for a specific reviewer assignment."""
    result = await db.execute(
        select(ReviewAssignment).where(
            ReviewAssignment.proposal_id == proposal_id,
            ReviewAssignment.reviewer_id == reviewer_id,
    ...
    await db.commit()
    return MessageResponse(message=f"Review deadline set to {payload.deadline}.")


@router.get("/proposals/{proposal_id}/review-status", response_model=List[ReviewAssignmentResponse])
async def get_review_status(
    proposal_id: int,
    current_user: User = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
    """Get the review status of all assignments for a proposal."""
    result = await db.execute(
        select(ReviewAssignment)
        .where(ReviewAssignment.proposal_id == proposal_id)
        .options(
            selectinload(ReviewAssignment.reviewer).selectinload(Reviewer.user),
            selectinload(ReviewAssignment.report),
        )
    )
    return result.scalars().all()
